# RF Asset Discovery Troubleshooting
...
**Symptom:** `rfad-survey resume <id>` doesn't continue

**Solutions:**

1. **Check survey status**
   ```bash
   rfad-survey status <survey_id>
   ```
   - `paused` - Can resume
...
3. **Use explicit database path**
   ```bash
   rfad-survey resume <id> --db data/unified.duckdb
   ```
...
3. **Manual intervention**
   ...
   - Resume with `rfad-survey resume`
